jQuery(document).ready(function($) {
	...
	function doStuff() {
		...
		.parent('li').click(function() {  //  get the parent li of the hidden ul's and when clicked...
			...
			$(this).parents('div.col').next('div.col').html(function() {  //  get this, find the parent div.col, then the next div.col and set the html...
				$(this).html(alanthicke);  //  take that next div.col and set it's html content to the variable we created on click
				doStuff();  //  then run this function again on the inserted content, this allows us to unlimited nested ul's
			}).nextAll('div.col').html('');  //  finally, clear the html contents of the next and all following div.col's
		});
	}
